...
Usage:
    python main.py
    Starts mitmproxy on 0.0.0.0:2130.
"""
...
if __name__ == "__main__":
    ...
    proc = subprocess.Popen(
        [
            sys.executable, "-c",
            f"import sys; from mitmproxy.tools.main import mitmdump; sys.argv = ['mitmdump', '-s', '{script_path}', '-p', '2130', '--listen-host', '0.0.0.0', '--set', 'block_global=false']; mitmdump()",
        ],
        env=os.environ.copy()
    )
    try:
