...
DEVELOPER_NAME = "JATIN"
DEVELOPER_NAME_HASH = hashlib.sha256(DEVELOPER_NAME.encode()).hexdigest()

# Hierarchy levels
class PermissionLevel:
    MAIN_OWNER = 1  # Can do everything, including adding other owners
    SUB_OWNER = 2   # Can only manage whitelists (add/remove UIDs)
    AUTHORIZED_USER = 5 # Can only manage whitelists
...
def load_config() -> dict:
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        default_config = {
            "main_owner_id": "1290868157749137409",  # Will be set when bot starts
...
class WhitelistBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        print(f"[System] Whitelist Bot developed by {DEVELOPER_NAME}")
...
def add_developer_footer(embed: discord.Embed) -> discord.Embed:
    """Add developer footer to embed."""
    embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME}")
    return embed

def create_error_embed(title: str, description: str) -> discord.Embed:
    """Create error embed with developer footer."""
    embed = discord.Embed(
        title=title,
...
def create_success_embed(title: str, description: str) -> discord.Embed:
    """Create success embed with developer footer."""
    embed = discord.Embed(
        title=title,
        description=description,
        color=0x2ecc71
...
def create_info_embed(title: str, description: str) -> discord.Embed:
    """Create info embed with developer footer."""
    embed = discord.Embed(
        title=title,
        description=description,
        color=0x3498db
...
async def owner_listpermissions(interaction: discord.Interaction):
    ...
    embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME} | Only Main Owner can modify these permissions")
...
async def whitelistadd(interaction: discord.Interaction, uid: str, server: str, duration: float = None):
    ...
    embed = discord.Embed(
        ...
        description=f"UID `{uid}` added to **{server.upper()}** whitelist and main whitelist.",
        color=0x2ecc71,
        timestamp=expiry_datetime
    )
    embed.add_field(name="📁 Whitelist Files", value=f"`whitelist_{server}.json`\n`whitelist.json`", inline=False)
    embed.add_field(name="⏳ Duration", value=f"`{actual_duration} hours` ({duration_source})", inline=True)
    embed.add_field(name="⏰ Expires", value=f"<t:{int(expiry_timestamp)}:F>", inline=True)
    embed.add_field(name="👤 Added by", value=f"<@{interaction.user.id}> ({level_names.get(adder_level, 'Unknown')})", inline=True)
    embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME} | Auto-removal when expired | Remove button below")
    ...
    async def remove_button_callback(button_interaction: discord.Interaction):
        ...
        updated_embed = discord.Embed(
            title="🗑️ UID Removed",
            description=f"UID `{uid}` removed from **{server.upper()}** whitelist and main whitelist.",
            color=0xe74c3c
        )
        updated_embed.add_field(name="👤 Removed by", value=f"<@{button_interaction.user.id}> ({level_names.get(remover_level, 'Unknown')})", inline=True)
        updated_embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME}")
        ...
        confirm_embed = discord.Embed(
            title="✅ UID Removed",
            description=f"UID `{uid}` removed from {server.upper()}",
            color=0x2ecc71
        )
        confirm_embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME}")
...
async def whiteliststatus(interaction: discord.Interaction, uid: str, server: str):
    ...
    embed.add_field(name="🌐 Server", value=f"`{server.upper()}`", inline=True)
    embed.add_field(name="📁 Whitelist Files", value=f"`whitelist_{server}.json`\n`whitelist.json`", inline=False)
    embed.add_field(name="⏳ Remaining", value=remaining_text, inline=False)
    
    if remaining < (999 * 365 * 24 * 3600):
        embed.add_field(name="⏰ Expires", value=f"<t:{int(expiry_timestamp)}:R>", inline=False)
...
async def whitelistlist(interaction: discord.Interaction, server: str):
    ...
    for idx, chunk in enumerate(chunks):
        embed = discord.Embed(
            title=f"📋 {server.upper()} Whitelist ({len(whitelist)} UIDs)",
            description="\n".join(chunk),
            color=0x9b59b6
        )
        
        embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME}" + (f" | Page {idx+1}/{len(chunks)}" if len(chunks) > 1 else ""))
    ...
    try:
        ...
        if "whitelisted_uids" in main_data and uid in main_data["whitelisted_uids"]:
            ...
            if remaining > 0:
                hours = remaining / 3600
                embed = discord.Embed(
                    title="✅ Found in Main Whitelist",
                    description=f"UID `{uid}` found in main whitelist.json",
                    color=0x2ecc71
                )
                embed.add_field(name="⏰ Expires", value=f"<t:{int(expiry)}:F>", inline=True)
                embed.add_field(name="⏳ Remaining", value=f"`{round(hours, 2)} hours`", inline=True)
                embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME}")
                await interaction.response.send_message(embed=embed, ephemeral=True)
            else:
...
async def whitelistaddall(interaction: discord.Interaction, uid: str, duration: float = None):
    ...
    embed = discord.Embed(
        title="✅ UID Whitelisted Globally",
        description=f"UID `{uid}` added to **ALL** server whitelists and main whitelist.",
        color=0x2ecc71,
        timestamp=expiry_datetime
    )
    embed.add_field(name="📁 Added to Files", value=f"{added_count} server files\n`whitelist.json`", inline=False)
    if failed_count > 0:
        embed.add_field(name="⚠️ Failed", value=f"{failed_count} files", inline=True)
    embed.add_field(name="⏳ Duration", value=f"`{actual_duration} hours`", inline=True)
    embed.add_field(name="⏰ Expires", value=f"<t:{int(expiry_timestamp)}:F>", inline=True)
    embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME}")
            ...
            try:
                ...
                if channel:
                    try:
                        ...
                        updated_embed = discord.Embed(
                            title="⌛ UID Expired",
                            description=f"UID `{data['uid']}` whitelist has expired on **{data['server'].upper()}**.",
                            color=0xf39c12
                        )
                        updated_embed.add_field(name="🌐 Server", value=data["server"].upper(), inline=True)
                        updated_embed.add_field(name="⏰ Expired", value=f"<t:{int(now)}:R>", inline=True)
                        updated_embed.set_footer(text=f"DEVELOPED BY {DEVELOPER_NAME}")
...
if __name__ == "__main__":
    ...
    bot.owner_id = 1360249820832600076  # CHANGE THIS TO YOUR DISCORD USER ID
